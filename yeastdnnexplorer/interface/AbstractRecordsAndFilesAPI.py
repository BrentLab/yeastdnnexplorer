...
class AbstractRecordsAndFilesAPI(AbstractAPI):
    """
    Abstract class to interact with both the records and the data stored in the
    `file` field. The return for this class must be records, against the `/export`
    ...
    def __init__(self, **kwargs):
        ...
        This will serve as an interface to an endpoint that can serve both records
        and files, and cache the file/retrieve from the cache if it exists.
    ...
    async def read(
        ...
        **kwargs,
        ...
        """
        Retrieve data from the endpoint according to the `retrieve_files` parameter.
        If `retrieve_files` is False, the records will be returned as a dataframe.
        If `retrieve_files` is True, the files associated with the records will be
    ...
    async def _retrieve_files(
        ...
        """
        Retrieve files associated with the records either from the local cache or
        from the database.
    ...
    async def _retrieve_file(
        ...
        """
        Retrieve a file associated with a record either from the local cache or
        from the database.
